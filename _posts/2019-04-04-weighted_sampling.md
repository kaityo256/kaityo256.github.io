---
layout: post
title: "重み付きランダムサンプリングアルゴリズム"
tags: [programming, qiita]
permalink: weighted_sampling
---
...
あらかじめ指定された重みに従って離散的な値を確率的に選択したい、ということがよくある。例えば`[1,4,5]`という配列が与えられたら、確率10%で0、40%で1、50%で2というインデックスを返すようにランダムサンプリングしたい。一番手軽な累積和の方法では、テーブル構築に$O(N)$、サンプリングに$O(\log(N))$の計算量となり、多くの場合これで用は足りる。さらに、テーブル構築は$O(N)$だが、サンプリングが$O(1)$となる[Walker's Alias法](https://kaityo256.github.io/walker_alias)という強力アルゴリズムもある。

累積和の方法は組むのが簡単で探索が$O(\log(N))$と十分早いし、Walker's Alias法は探索が$O(1)$と強烈なので、「重み付きランダムサンプリングアルゴリズムは、もうこの２つのどちらかで十分じゃん？」という気持ちになるが、この2つのアルゴリズムでは、重みの部分書き換えができない、という弱点がある。つまり、どこか一つの配列の重みが変更された場合、テーブル構築にまた$O(N)$の手間がかかってしまう。
...
![image0.png](/assets/images/weighted_sampling/image0.png)
...
![image1.png](/assets/images/weighted_sampling/image1.png)
...
の2ステップでインデックスを選べる。どんなに$N$が大きくても2ステップでインデックスを選べるので、$O(1)$の方法となっていることがわかるであろう。この箱の作り方がやや難しいのだが、詳細は[別記事](https://kaityo256.github.io/walker_alias)を参照して欲しい。
...
![image2.png](/assets/images/weighted_sampling/image2.png)
...
![image3.png](/assets/images/weighted_sampling/image3.png)
