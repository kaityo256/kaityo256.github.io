...
![image0.png](/assets/images/cpu_keywords/image0.png)
...
![image1.png](/assets/images/cpu_keywords/image1.png)
...
ARMは長らく32ビットアーキテクチャとして発展してきましたが、ARMv8から64ビットアーキテクチャが定義されました。ARMv8は32ビットと64ビット両方のアーキテクチャを含み、64ビットに対応しているアーキテクチャはARMv8-A、32ビットにのみ対応しているアーキテクチャはARMv8-Rです(注：追記参照)。
...
## (2020年9月15日追記) ARMv8-RのAArch64対応について

コメント欄にて「ARMv8-Aでも32ビットしか対応していないものがあり、ARMv8-Rでも64ビット対応のものがある」という指摘がありました。まず、ARMはv7より「A/R/M」という三つのプロファイルが定義されました。それぞれ「A:アプリケーション向け」「R:リアルタイム向け」「M:組み込み向け」です。このうち組み込み向けは良く知らないのでAとRの話だけ書きます。

ARMv8が発表された時(2011年)、ARMv8-Aが64ビット対応しました。これは命令セットとして64ビットモードが定義されたということで、コメント欄にあるようにARMv8-Aが64ビットをサポートするかどうかは実装者に任されます。

さて、リアルタイム向けのISAであるR系統のVersion 8、ARMv8-Rですが、発表当初はARMv7-Rのマイナーアップデートという感じで、対応する製品もありませんでした。ARMv7では64ビットに対応していなかったため、ARMv8-Rとなっても当然32ビットのみでした。しかし、最近(2020年9月3日？)発表されたCortex-R82では、AArch64に対応することになりました。SIMD命令セットであるNEONにも対応するなど、僕にはARMv8-Aとの違いがよくわかりません。そのあたりは識者による記事を待ちます。
