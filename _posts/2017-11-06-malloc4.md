...
* [mallocの動作を追いかける(fastbins編)](https://kaityo256.github.io/malloc4) ← イマココ
* [mallocの動作を追いかける(マルチスレッド編)](https://kaityo256.github.io/malloc5) 
* [mallocの動作を追いかける(環境変数編)](https://kaityo256.github.io/malloc6)
...
この`struct malloc_chunk* fd`が、ちょうどmallocが返してくるポインタの位置である。mallocの返してくる位置と、チャンクで管理される場所がずれているのは[以前の記事](https://kaityo256.github.io/malloc2)で見たとおり。小さいサイズのチャンクがfreeされる時、この`fd`の位置に「一つ前のチャンク」の位置がセットされる。小さいサイズでは`fd`しか使わない前方参照リンクになっている。図解するとこんな感じになっている。
...
さて、fastbinsは24バイトから16バイトずつ管理されており、配列としては10個用意されているため、24,40,...,168バイトまでのビンがありそうな気がする。それを確認するため、こんなコードを書いてみよう。まずは[前回](https://kaityo256.github.io/malloc3)も定義した`main_arena`表示用の構造体定義を作る。
...
[続く](https://kaityo256.github.io/malloc5)
