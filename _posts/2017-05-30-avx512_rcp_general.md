...
と、加減乗算のみで構成できた。無論、$m=1$を代入すれば[逆数近似の補正公式](https://kaityo256.github.io/avx512_rcp)に一致する。

## 逆数平方根近似

というわけで逆数平方根近似を試す。まずは無補正。コードはほとんど[逆数近似の記事](https://kaityo256.github.io/avx512_rcp)と同じだが、`__m512d`の型でやると計算をいちいち組み込みを使う必要があって面倒なので
