...
一自由度系$(p,q)$を考える。これがある変換(時間発展)により、新たな変数の組$(P,Q)$になったとする。この写像がシンプレクティックであるとは、この写像がシンプレクティック二形式を保存することである([参考](https://kaityo256.github.io/velocity_verlet))。これは要するに、$(p,q)$から$(P,Q)$への変数変換のヤコビアンが1であることと等価である。
